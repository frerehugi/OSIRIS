...
carried over, what didn't (and why), and whether an activation path actually exists.
See `src/reference/README.md` for the index.

| File | Ported from | What it is |
|---|---|---|
| `paymentRequestQr.ts` | [`Investorquab/CeloDesk`](https://github.com/Investorquab/CeloDesk) (public repo), `frontend/components/CheckoutClient.tsx`'s `paymentQrValue()` | Builds an EIP-681 `ethereum:<token>@<chainId>/transfer?...` payment-request URI for a scannable QR — MiniPay and other EIP-681-aware wallets read it to prefill a token transfer. Reviewed 21.09.2026: CeloDesk itself has no custom MiniPay deeplink scheme (relies on the standard injected `window.ethereum` provider) and no camera-based QR *scanning* (only generates QR codes to display, via `qrcode.react`). Its `wallet_switchEthereumChain` chain-switch dance was deliberately **not** ported: `minipayWallet.ts` already documents that MiniPay is Celo-only and doesn't support programmatic chain switching, so that logic would be dead code here. **Corrected 21.09.2026: no identified use case.** An initial claim that this could power a "scan to fund your vault" screen was wrong — `DcaVault.sol`'s `setupPlan()` (and Send/TriggerVault's equivalents) pull the input token via `inputToken.safeTransferFrom(owner, address(this), _totalAmount)` in the same signed session as `approve()`, with no `topUp()`; tokens sent to a vault address any other way sit unaccounted by the plan. Checked against Sterntaler (identical `createVault()→approve()→setupPlan()` pattern per its README) and Clock-it (moves no tokens at all — its own QR encodes a location, see its `src/lib/qr.ts`): none of the three has a moment where handing someone a scannable payment request helps, since money always moves either as an owner's own pull in one wallet session or as an automated keeper push. Kept dormant, logic-correct, in case a future feature actually creates that moment. |
